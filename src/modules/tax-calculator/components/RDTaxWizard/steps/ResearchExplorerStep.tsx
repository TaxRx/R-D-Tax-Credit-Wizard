...
const ResearchExplorerStep: React.FC<ResearchExplorerStepProps> = ({
  ...
  }, [parentSelectedYear, selectedYear, availableBusinessYears]);
  ...
  // Load available business years for the current business (simplified version for demo)
  const loadAvailableBusinessYears = async () => {
    try {
      console.log('Loading available business years (demo mode)');
      
      // For demo purposes, generate years from 2020 to 2026
      const currentYear = new Date().getFullYear();
      const demoBusinessYears = [];
      
      for (let year = 2020; year <= currentYear + 2; year++) {
        demoBusinessYears.push({
          id: `demo-year-${year}`,
          year: year
        });
      }
      
      console.log('Demo business years:', demoBusinessYears);
      setAvailableBusinessYears(demoBusinessYears);
    } catch (error) {
